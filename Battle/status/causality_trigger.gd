...
func _init(p_state_conditions: Array[StateCondition] = [], 
			p_event_conditions: Array[EventCondition] = [], p_modifiers: Array[Modifier] = [], 
			p_amount: int = -1, p_reset: Reset = Reset.PER_BATTLE):
	state_conditions = p_state_conditions
	event_conditions = p_event_conditions
...
func get_description():
	...
	var mod_text: String = ""
	var first_pass_event: bool = true
#	var start: String = "If:" if event_condition is EventCondition else "When:"
	for state in state_conditions:
		if cond_text == "": cond_text += "If:" if not event_conditions.is_empty() else "When:"
#		if cond_text != "": cond_text += "\n\t"
		cond_text += "\n* " + state.get_description()
	for event_condition in event_conditions:
		if first_pass_event:
			cond_text += "\nWhen: \n* " + event_condition.get_description()
			first_pass_event = false
		else:
			cond_text += " or\n* " + event_condition.get_description()
